...
PARAMS = {
    ...
    # 'dir': 'bead2', 'num_channels': 1, 'raw_input_shape': (128,128), 'target_input_shape': (128,128),
    ...
    'chpt': 151,
    'channel_format': 'NCHW',
    'load_chpt': 151, # Checkpoint number of model you would like to load or None for new model
...
class Generate_Brain_Ref:
    ...
    def setup_model(self):
        self.F = tf.keras.models.load_model('./models/' + str(PARAMS['load_chpt']) + '/F', compile = True)
    ...
    def GenerateBrainRef(self):
        ...
        self.display_test_images(-2, brain_gen, num=333, ccm_only=True)

        pfa_brain_gen = ds.setup_pfa_brain_eval_set()
        self.display_test_images(-2, pfa_brain_gen, num=173, ccm_only=True, pfa=True)
...
def main():
    brain = Generate_Brain_Ref()
    brain.GenerateBrainRef()
