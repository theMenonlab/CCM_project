...
class DataSetup:
    ...
    def __get_glia_imgs(self):
        x, y = [], []
        for i in range(21648): #21648
        	#left_01112022
            x.append('./datasets/glia/refL_glia_02062022/' + str(i) + '.jpeg')
            y.append('./datasets/glia/ccmL_glia_02062022/' + str(i) + '.jpeg')
        return x, y          
    ...
    def __get_brain_test_imgs(self):
        y = []
        for i in range(334):
            y.append('./datasets/fresh_brain/ccmL_fresh_0127/' + str(i) + '.jpeg')
        return y
    def __get_pfa_brain_test_imgs(self):
        y = []
        for i in range(173):
            y.append('./datasets/pfa_brain/ccmL_pfa_0127/' + str(i) + '.jpeg')
        return y
    ...
    def __get_bead_imgs(self):
        x, y = [], []
        for i in range(14410):
            x.append('./datasets/bead/Ref_L/' + str(i) + '.jpeg')
            y.append('./datasets/bead/CCM_L/' + str(i) + '.jpeg')
        return x, y
    ...
    def setup_datasets(self):      
        ...
        if self.data_path == 'cannula-neuron':
        ...
        elif self.data_path == 'glia':
            x, y = self.__get_glia_imgs()
            train_x = x[3000:]
            val_x = x[1000:3000] 
            test_x = x[0:1000] 
            ...
            preprocess = preprocess_glia
            preprocess_x, preprocess_y = preprocess_glia(train_x[0], train_y[0])
    ...
    def setup_pfa_brain_eval_set(self):
        y = self.__get_pfa_brain_test_imgs()
        test_ds = tf.data.Dataset.from_tensor_slices((y))
        test_ds = test_ds.map(preprocess_pfa_brain_eval, num_parallel_calls=tf.data.experimental.AUTOTUNE).batch(self.batch_size).prefetch(tf.data.experimental.AUTOTUNE)
        return test_ds
...
def preprocess_pfa_brain_eval(y):
    y = tf.io.read_file(y)
    y = tf.image.decode_jpeg(y, channels=1)
    y = tf.cast(y, tf.float32)
    y = tf.image.resize(y, (128,128))
...
def generate_images_brain(F, data_gen, chpt, batch_size, num=1, num_channels=3, train_str='train', pfa=False):
    displayed = 0
    for y in data_gen:
        f_prediction = F(y, training=False)
        ...
        for j in range(batch_size):
            ...
            for i in range(2):
                ...
                if num_channels == 1:
                    ...
                    if not pfa:
                    		...
                    		skimage.io.imsave('./figures/' + str(chpt) + '/pfa_brain/' + train_str + '-' + str(displayed) + '-' + fig_title[i] + '.png', img)
                else:
                    img = display_list[i]
            ...
            if not pfa:
            	...
            	plt.savefig('./figures/' + str(chpt) + '/pfa_brain-' + train_str + '-' + str(displayed), bbox_inches='tight', pad_inches=0)
            displayed += 1
            if displayed == num: return
